...
<template>
  ...
  <div v-if="isLoading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    <div v-for="i in 6" :key="i" class="event-card rounded-xl shadow-lg animate-pulse">
  ...
  <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    >
      ...
      <div class="relative overflow-hidden">
        ...
        <!-- Category Badge -->
        <div v-if="event.category" class="absolute bottom-3 right-3 bg-[var(--color-secondary)]/90 px-2 py-1 rounded-full text-xs font-medium text-white">
          {{ event.category }}
        </div>

        <!-- Urgency Indicator -->
        <div v-if="event.spotsLeft && event.spotsLeft < 20" class="absolute bottom-3 left-3 bg-[var(--color-error)] px-2 py-1 rounded-full text-xs font-medium animate-pulse text-white">
      ...
      <div class="p-4">
        ...
        <div class="flex justify-between items-start mb-2">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white group-hover:text-[var(--color-primary)] transition-colors line-clamp-1">
            {{ event.title }}
          </h3>
          <div v-if="event.rating" class="flex items-center gap-1 text-sm ml-2 flex-shrink-0">
            <Star class="w-4 h-4 fill-[var(--color-primary)] text-[var(--color-primary)]" />
            <span class="text-gray-600 dark:text-gray-300">{{ event.rating }}</span>
          </div>
        </div>
        ...
        <div class="space-y-2 mb-3">
          <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
            <Calendar class="w-4 h-4 text-[var(--color-primary)] flex-shrink-0" />
            <span>{{ formatDate(event.date) }}</span>
            <Clock class="w-4 h-4 text-[var(--color-primary)] ml-2 flex-shrink-0" />
        ...
        <!-- Event Description -->
        <p class="text-gray-600 dark:text-gray-300 text-sm mb-3 line-clamp-2">{{ event.description }}</p>

        <!-- Organizer Info -->
        <div v-if="event.organizer" class="flex items-center gap-2 mb-3 text-xs text-gray-600 dark:text-gray-300">
          <img 
